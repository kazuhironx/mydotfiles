---
name: when-stuck
description: Auto-triggers when blocked — 2+ failed fix attempts on the same issue, retry loops, repeated "not sure why", or visibly stalled progress. Dispatches to the matching problem-solving technique (simplification / collision-zone / meta-pattern / inversion / scale-game / systematic-debugging) based on stuck-type. Skip when progress is being made even slowly.
