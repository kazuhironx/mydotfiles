...
description: Auto-triggers when investigating a bug, failing test, build failure, or unexpected behavior — before forming any fix hypothesis. Enforces a 4-phase root-cause-first workflow (investigate → pattern → hypothesis → fix). Skip for obvious syntax errors or typos pinned to a single error line.
